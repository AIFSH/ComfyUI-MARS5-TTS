...
class Mars5TTS(nn.Module, ModelHubMixin):
    ...
    def _from_pretrained(
        ...
        **model_kwargs,
        ...
        ar_ckpt_path = hf_hub_download(repo_id=model_id, filename="mars5_ar.safetensors", revision=revision, local_dir=local_dir, force_download=force_download, proxies=proxies, local_files_only=local_files_only, token=token)
        logging.info(f">>>>> Downloading NAR model")
        nar_ckpt_path = hf_hub_download(repo_id=model_id, filename="mars5_nar.safetensors", revision=revision, local_dir=local_dir, force_download=force_download, proxies=proxies, local_files_only=local_files_only, token=token)
